...
public class ReadFeedbackActivity {
    ...
    public FeedbackRepresentation read(AppealsUri feedbackUri) {
        ...
        Comments comment = CommentsRepository.current().get(identifier);
        
        return new FeedbackRepresentation(comment, new Link(Representation.RELATIONS_URI + "appeals", UriExchange.appealForFeedback(feedbackUri)));
    }
